...
def test_employers_are_not_on_grid():
    ...
    employers_list = [agent for agent in model3.schedule.agents if isinstance(agent, Employer)]
    assert all(agent not in model3.get_all_agents_on_grid() for agent in employers_list)


def test_people_are_on_grid():
    model4 = Model()
    model4.schedule = RandomActivation(model4)
    model4._put_people_in_model(initial_money)
    people_list = [agent for agent in model4.schedule.agents if isinstance(agent, Person)]
    assert all(agent in model4.get_all_agents_on_grid() for agent in people_list)
